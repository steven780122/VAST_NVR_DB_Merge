class CctNvrDb::CamerasController < ApplicationController
  ...
  def create
    ...
    new_cam_objs = CctNvrDb::ShowCamInfoAndUpdate.find(new_cam_ids_array)
    Rails.logger.debug("TEST new cam objs from string HASH!!: #{new_cam_objs.inspect}")   # ok

    added_soc_hash = CctNvrDb::ShowCamInfoAndUpdate.get_selected_to_add_cam_hash_array(new_cam_objs, params[:soc_option])
    ...
    CctNvrDb::Camera.create(added_soc_hash)

    redirect_to new_and_update_cam_info_index_cct_nvr_db_cameras_path
  ...
  def update
    ...
    CctNvrDb::Camera.update(get_id_key, get_cam_value_hash_array)
  ...
  def update_soc
    ...
    cam_soc_to_updated_hash_hash = CctNvrDb::ShowCamInfoAndUpdate.get_selected_to_update_soc_hash_hash(soc_to_be_updated_id_array, soc_option_str)
    Rails.logger.debug("TEST SOC FINAL PARAMS: #{cam_soc_to_updated_hash_hash.inspect}")   # ok

    CctNvrDb::Camera.update(cam_soc_to_updated_hash_hash.keys, cam_soc_to_updated_hash_hash.values)
  ...
  def show_cam_info_index
    @cameras = CctNvrDb::Camera.all
    @model_name_array = CctNvrDb::Camera.get_model_name_array(@cameras)
    @soc_array = CctNvrDb::TestSocCamera.show_array_of_soc_option()

    # if search by soc option
    if params[:search_by_soc_option]
      @selected_keyword_models = CctNvrDb::Camera.search_by_soc(params[:search_by_soc_option])
    end

    # if search by keyword
    if params[:search_update_cam_by_keyword]
      @selected_keyword_models = CctNvrDb::Camera.search_by_keyword(params[:search_update_cam_by_keyword])
    end
  end
  ...
  def show_cam_info
    ...
    if params[:check_box_ids_array]
      ...
      @checked_model_array = CctNvrDb::Camera.find(params[:check_box_ids_array])
      ...
      @db_camera_name_obj_array = CctNvrDb::ShowCamInfoAndUpdate.get_db_camera_name_obj_array(@checked_model_array)
  ...
  def show_soc_info_index
    @soc_array = CctNvrDb::TestSocCamera.show_array_of_soc_option()
    @test_cam_array = CctNvrDb::TestSocCamera.get_test_camera_model_array()


    # the following is processing
    if params[:search_compare_cam_by_keyword]
      @imported_keyword_models = CctNvrDb::TestSocCamera.search_by_keyword(params[:search_compare_cam_by_keyword])
    end
  ...
  def show_soc_info
    ...
    if params[:check_box_ids_array]
      ...
      # soc info
      @db_cameras_by_soc = CctNvrDb::Camera.search_soc(params[:search_soc])
      ...
      CctNvrDb::Camera.column_names.each do |column_name|
        ...
        if column_name == "id" || column_name == "created_at" || column_name == "updated_at"
          ...
          instance_variable_set("@db_#{column_name}", CctNvrDb::Camera.deal_supported_by_column_name(@db_cameras_by_soc, column_name))
        end
      ...
      @test_selected_camera_array = CctNvrDb::TestSocCamera.find(params[:check_box_ids_array])
      @selected_indexed_hash = CctNvrDb::TestSocCamera.set_selected_compare_cam_to_hash(@test_selected_camera_array)
  ...
  def new_and_update_cam_info_index
    @import_update_cameras = CctNvrDb::ShowCamInfoAndUpdate.all

    # get import model name array
    @new_import_model_name_array = CctNvrDb::ShowCamInfoAndUpdate.get_model_name_array(@import_update_cameras)

    # get camera to be updated
    @update_cam_array = CctNvrDb::Camera.get_update_cam_array()
  ...
  def new_cam_info_index
    ...
    if params[:search_update_cam_by_keyword]
      # new ver.
      @imported_keyword_models = CctNvrDb::ShowCamInfoAndUpdate.search_by_keyword(params[:search_update_cam_by_keyword])
  ...
  def new_cam_info
    ...
    if params[:check_box_ids_array]
      # @checked_model_array = params[:check_box_ids_array]
      @checked_model_array = CctNvrDb::Camera.where(:Model_FW_name => params[:check_box_ids_array])

    end

    # test for update
    @cam_add_targets = CctNvrDb::ShowCamInfoAndUpdate.find(params[:check_box_ids_array])
    @soc_array = CctNvrDb::TestSocCamera.show_array_of_soc_option()
  ...
  def update_cam_info
    ...
    @db_camera_name_obj_array = CctNvrDb::ShowCamInfoAndUpdate.get_db_camera_name_obj_array(@checked_model_array)
    @import_cameras_obj_array = CctNvrDb::ShowCamInfoAndUpdate.get_import_cameras_obj_array(@import_update_cameras, @checked_model_name_str_array)
  ...
  def update_cam_other_info_index
    @camera_other_info = CctNvrDb::CameraOtherInfo.new
    @import_update_cameras = CctNvrDb::ShowCamInfoAndUpdate.all
    @cameras = CctNvrDb::Camera.all

    @soc_array = @soc_array = CctNvrDb::TestSocCamera.show_array_of_soc_option()

    # get import model name array
    @new_import_model_name_array = CctNvrDb::ShowCamInfoAndUpdate.get_model_name_array(@cameras)
  ...
  def update_cam_other_info
    ...
    if params[:check_box_ids_array]
      ...
      @checked_model_id_array.each do |cheked_model_id|
        cheked_model_obj = CctNvrDb::Camera.find(cheked_model_id)
  ...
  def create_camera_other_info
    @checked_model_for_other_info_name_str_array = params[:selected_model_name]
    ...
    Rails.logger.debug("hidden selectedND9541: #{params[:'ND9541']}")
    Rails.logger.debug("hidden selectedND8322P: #{params[:ND8322P]}")
    Rails.logger.debug("hidden selectedShepherd: #{params[:'Shepherd']}")

    Rails.logger.debug("hidden selectedND9541_box: #{params[:'ND9541_box']}")
    Rails.logger.debug("hidden selectedND8322P_box: #{params[:ND8322P_box]}")
    Rails.logger.debug("hidden selectedShepherd_box: #{params[:'Shepherd_box']}")

    Rails.logger.debug("hidden selectedTEST1: #{params[:TEST1]}")
    # if checked, the value will be 1
    ...
    Rails.logger.debug("sw nvr array str!!!!: #{params[:sw_nvr_array]}")
    all_sw_nvr_str = params[:sw_nvr_array]
    # Rails.logger.debug("sw nvr array test_type!!!!: #{all_sw_nvr_str.class}")       # str
    all_sw_nvr_array = all_sw_nvr_str.split(' ')

    issue_sw_nvr_array = []
    checkbox_checked_array = []
    total_cam_other_info_hash_array = []

    all_sw_nvr_array.each do |sw_nvr|
      Rails.logger.debug("hidden selectedsw_nvr: #{sw_nvr}")

      sw_nvr_box_str = sw_nvr + '_box'
      Rails.logger.debug("hidden selectedsw_nvr_box_str: #{params[sw_nvr_box_str]}")

      # if specific checkbox is not selected, it will put nil inside so we need to avoid it
      unless params[sw_nvr_box_str].nil?
        checkbox_checked_array.append(params[sw_nvr_box_str])
      end

      sw_nvr_box_str_existed = FALSE
      if !params[sw_nvr_box_str].nil?
        sw_nvr_box_str_existed = TRUE
      end

      sw_nvr_existed = FALSE
      if !params[sw_nvr].to_s.strip.empty?
        sw_nvr_existed = TRUE
      end

      # Rails.logger.debug("params[sw_nvr_box_str]: #{params[sw_nvr].class}")

      # Rails.logger.debug("exclusive-nvr-box-str: #{params[sw_nvr]^params[sw_nvr_box_str]}  _ #{params[sw_nvr]}")
      Rails.logger.debug("exclusive-nvr-box-str: #{sw_nvr_box_str_existed^sw_nvr_existed}  _ #{params[sw_nvr]}")

      has_sw_nvr_issue_bool = sw_nvr_box_str_existed^sw_nvr_existed

      if has_sw_nvr_issue_bool
        issue_sw_nvr_array.append(sw_nvr)
      else   # append every hash into array
        copied_cam_other_info_hash = temp_camera_other_info_without_model_name_hash.clone

        # if specific has no issue, but it is not selected
        unless params[sw_nvr_box_str].nil?
          copied_cam_other_info_hash["SW_NVR"] = sw_nvr
          copied_cam_other_info_hash["SW_NVR_Ver"] = params[sw_nvr]
          total_cam_other_info_hash_array.append(copied_cam_other_info_hash)
        end
      end
    end

    Rails.logger.debug("issue_sw_nvr_array: #{issue_sw_nvr_array}")
    Rails.logger.debug("issue_sw_nvr_array: #{issue_sw_nvr_array.size}")
    Rails.logger.debug("checkbox_checked_array: #{checkbox_checked_array}")
    Rails.logger.debug("checkbox_checked_array_size: #{checkbox_checked_array.size}")

    # exist issue
    if issue_sw_nvr_array.size > 0
      flash[:issue_sw_nvr_array_notice] = "#{issue_sw_nvr_array.join(",")}的SW/NVR資訊有誤，請重新填寫！"
      redirect_to :back
      return
    end

    if checkbox_checked_array.size == 0
      flash[:issue_sw_nvr_array_notice] = "未填寫SW/NVR資訊，請重新填寫！"
      redirect_to :back
      return
    end


    Rails.logger.debug("total_cam_other_info_hash_array!!!!: #{total_cam_other_info_hash_array}")

    total_cam_other_info_hash_array.each do |cam_other_info_hash|

      # add_camera_other_info_hash_array = CctNvrDb::CameraOtherInfo.get_add_camera_other_info_hash_array(
      #     selected_model_name_hidden_str_array, temp_camera_other_info_without_model_name_hash)

      add_camera_other_info_hash_array = CctNvrDb::CameraOtherInfo.get_add_camera_other_info_hash_array(
          selected_model_name_hidden_str_array, cam_other_info_hash)
      ...
      if !add_camera_other_info_hash_array.nil?
        CctNvrDb::CameraOtherInfo.create(add_camera_other_info_hash_array)
        flash[:new_and_update_notice] = "Camera Other Info新增成功"
      else
        flash[:new_and_update_notice] = "Camera Other Info新增失敗!!"
        redirect_to update_cam_other_info_index_cct_nvr_db_cameras_path
        return
      end
