class CctVastDb::ShowCamInfoAndUpdate < ActiveRecord::Base
  ...
  def self.import(file)
    CctVastDb::ShowCamInfoAndUpdate.destroy_all
    ...
    CSV.foreach(file.path, headers:true) do |row|
      ...
      CctVastDb::ShowCamInfoAndUpdate.create! new_hash.slice(
          "CameraModel",
          "Numberofstreams",
          "Videocodecsupported",
          "FOVsupported",
      ...
      # ShowCamInfoAndUpdate.create! row.to_hash
    end
  end
  ...
  def self.get_import_cameras_obj_array(import_update_cameras, checked_model_name_str_array)
    ...
    import_update_cameras.each do |import_update_camera_obj|
      if checked_model_name_str_array.include? import_update_camera_obj["CameraModel"]
  ...
  # the output we need is like: [{:CameraModel: "XXX1", :Soc : "s1"},{:CameraModel: "XXX2", :Soc : "s2"}]
  def self.get_selected_to_add_cam_hash_array(selected_cam_obj_instances, soc_name_str)
    ...
    selected_cam_obj_instances.each do |selected_cam_obj|
      ...
      if !CctVastDb::Camera.find_by_CameraModel(selected_cam_obj["CameraModel"])
  ...
  def self.get_selected_to_update_cam_without_soc_hash_hash(db_cam_updated_target_objs_array, import_update_sources_objs_array)
    ...
    db_cam_updated_target_objs_array.each do |updated_target_cam_obj|
      ...
      target_cam_name_str = updated_target_cam_obj["CameraModel"]

      temp_cam_info_for_updated_hash = {}

      import_update_sources_objs_array.each do |import_source_cam_obj|
        if import_source_cam_obj["CameraModel"] == target_cam_name_str
