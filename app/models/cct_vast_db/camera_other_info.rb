class CctVastDb::CameraOtherInfo < ActiveRecord::Base

  require 'csv'
  validates_presence_of :"CameraModel"     # "Model name" must be written
  belongs_to :cct_vast_db_camera
  # , :class_name => 'CctVastDb::Camera'
  ...
  def self.import(file)
    ...
    CSV.foreach(file.path, headers:true) do |row|
      ...
      CctVastDb::CameraOtherInfo.create! row.to_hash    # ori OK!! (but no association when create db!)
  ...
  def self.update_association_foreign_key
    ...
    camera_other_infos_obj.each do |camera_other_info|
      ...
      match_camera_obj = CctVastDb::Camera.find_by_CameraModel(camera_other_info_model_name)
  ...
  def self.get_add_camera_other_info_hash_array(
    ...
    if !selected_model_name_hidden_str_array.empty?
      ...
      selected_model_name_hidden_str_array.each do |selected_model_name_hidden_str|
        ...
        temp_model_name_hash[:cct_vast_db_camera_id] = CctVastDb::Camera.find_by_CameraModel(selected_model_name_hidden_str).id
  ...
  def self.check_all_other_info_model_existed()
    ...
    if CctVastDb::CameraOtherInfo.exists?
      total_camera_other_infos = CctVastDb::CameraOtherInfo.all
      total_camera_infos = CctVastDb::Camera.all
